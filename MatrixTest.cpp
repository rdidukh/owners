...
#if 0
static void readMatrixFileTest()
{
	Matrix<double> mtx;
	bool ret;

	mtx.push_back(MatrixRow<double>());
	ret = readMatrixFile("unexisting.file", mtx);
	ASSERT_FALSE(ret);
	ASSERT_EQ(0, mtx.size());

	std::ofstream testFile("test.mtx", std::ofstream::trunc);
	testFile << "3 3 1 2 3 6 5 4 7 8";
	testFile.close();

	ret = readMatrixFile("test.mtx", mtx);
	ASSERT_FALSE(ret);
	ASSERT_EQ(0, mtx.size());

	testFile.open("test.mtx", std::ofstream::trunc);
	testFile << "2 3 0.1 1.0 0.56  0.32 0.99 .0";
	testFile.close();
	...
	ASSERT_EQ(3, mtx[1].size());
	ASSERT_EQ_F(0.1, mtx[0][0], 0.001);
	ASSERT_EQ_F(1.0, mtx[0][1], 0.001);
	ASSERT_EQ_F(0.56, mtx[0][2], 0.001);
	ASSERT_EQ_F(0.32, mtx[1][0], 0.001);
	ASSERT_EQ_F(0.99, mtx[1][1], 0.001);
	ASSERT_EQ_F(0.0, mtx[1][2], 0.001);
}
#endif 
